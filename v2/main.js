...
function init() {
	...
	function initMatchGame(targetSVG) {
		...
		function handleGameClose() {
			...
			t.to(blackOverlay, 0.3, {opacity:0, ease:Power2.easeOut, delay:0.2, onComplete:overlayDepth});
			clearInterval(countDownTimer);
			function overlayDepth() {
				blackOverlay.css({ zIndex : -10 });
		...
		function handleEnd(targetSVG, won) {
			...
			t.to(overlay, 0.3, {opacity:0, onComplete:resetOverlay});

			function resetOverlay() {
				overlay.html('');
				overlay.append(winScreenDom);
				t.to(overlay, 0.3, {opacity:1});
			...
			function parallaxBh() {
				t.to('#wsBg', 4, {backgroundSize:'100% 100%', 'filter' : 'blur(2px)', ease:Expo.easeOut});
				t.to('#wsFg', 8, {backgroundSize:'100% 100%', x:'-=200', y:'-=40', ease:Expo.easeOut});
			};

			function handleMouseMove() {
				//console.log();

				var ol = $('#wsBg').offset().left;
				var fol = $('#wsFg').offset().left;


				$(document).mousemove( function(e) {
					console.log(e.clientX);

					$('#wsFg').css({ backgroundPosition : (e.clientX - window.innerWidth / 2) * 0.05   })
					$('#wsBg').css({ backgroundPosition : (e.clientX - window.innerWidth / 2) * 0.01   })


					//$('#wsBg').offset({ left : ol - (e.clientX - window.innerWidth / 2) * 0.05   })
					//$('#wsFg').offset({ left : fol - (e.clientX - window.innerWidth / 2) * 0.005   })
